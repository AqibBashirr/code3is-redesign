...
export default function BlogShare({ title, description = "" }: BlogShareProps) {
  ...
  return (
    <div ref={wrapperRef} className="relative mt-6 inline-flex">
      ...
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className=" group hover:text-offBlack-color inline-flex items-center gap-2 rounded-full border border-[#3A3B3A] bg-card-color px-5 py-2.5 text-sm font-medium text-highlight-text-color transition-all duration-300 hover:-translate-y-0.5 hover:border-primary-color hover:bg-primary-color/10"
      >
      ...
      <div
        role="menu"
        aria-hidden={!open}
        className={`absolute left-0 top-full z-50 mt-4 w-72 origin-top-left overflow-hidden rounded-2xl border border-white/10 bg-card-color/95 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-all duration-300 ${open ? "translate-y-0 scale-100 opacity-100" : "-translate-y-2 scale-95 opacity-0 pointer-events-none"}
        `}
      >
        {/* Arrow */}
